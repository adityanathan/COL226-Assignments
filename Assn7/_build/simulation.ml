...
let rec simulation call_stack dump display_reg=
  ...
  | Display -> print_stack call_stack; print_disp (List.rev display_reg); simulation call_stack dump display_reg
  | Set(var_name, value) -> let current_pr = !^(List.hd call_stack) in
    (match current_pr with
       (Proc(a,b,c,d,e,f),tabl) ->let _ = ((List.hd call_stack) ^:= (Proc(a,b,c,d,e,f),replace_var_from_table var_name (substitute_variable value tabl) tabl)) in simulation call_stack dump display_reg)
