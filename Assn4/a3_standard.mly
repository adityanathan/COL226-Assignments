...
%token TRUE FALSE LP RP IF THEN ELSE DEF DELIMITER EOF EQ GTA LTA GEQ LEQ ABS EXP DIV MOD MUL PLUS MINUS AND OR NOT
%token <int> INT
%token <string> ID
%token <bool> BOOL
...
ifte_def_expr:
    IF sub_expr THEN sub_expr ELSE sub_expr     {IfThenElse($2,$5,$7)}
    | sub_expr                                  {$1}
    /* | DEF   */
