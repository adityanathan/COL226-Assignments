...
let minus (x : bigint) =
  ...
  | NonNeg, e -> if e <> [] then ((Neg, e) : bigint) else ((NonNeg, e) : bigint)
...
let rec gt_prototype x y =
  if len x > len y then true
    ...
    | [], [] -> false
    | h1 :: t1, h2 :: t2 ->
        if h1 > h2 then true else if h1 = h2 then gt_prototype t1 t2 else false
...
let rec lt_prototype x y =
  if len x > len y then false
    ...
    | [], [] -> false
    | h1 :: t1, h2 :: t2 ->
        if h1 < h2 then true else if h1 = h2 then lt_prototype t1 t2 else false
...
let rec mult_prototype (x : int list) (y : int list) (acc : int list) (i : int)
    =
  match y with
  | [] -> acc
...
let rec div_prototype x y quotient i =
  ...
  if lt a b && i = -1 then quotient
    ...
    let rem =
      get_list
        (sub (NonNeg, head) (mult (NonNeg, trim_list [div_single head y 0]) b))
    in
  ...
  | (Neg, l1), (NonNeg, l2) ->
      if
        not
          (eq
             (mult
                (Neg, div_prototype l1 l2 [] (len (get_list x) - 1))
                (NonNeg, l2))
             (Neg, l1))
        (* This if clause checks whether l1 is perfectly divisible by l2 or not.
        If it is in this case we subtract the quotient by 1 so that the remainder is positive *)
      then
        trim
          ( sub (Neg, div_prototype l1 l2 [] (len (get_list x) - 1)) (mk_big 1)
            : bigint )
      else trim ((Neg, div_prototype l1 l2 [] (len (get_list x) - 1)) : bigint)
  (* 1 is subtracted to make sure remainder cannot be negative *)
  | (Neg, l1), (Neg, l2) ->
      trim ((NonNeg, div_prototype l1 l2 [] (len (get_list x) - 1)) : bigint)
